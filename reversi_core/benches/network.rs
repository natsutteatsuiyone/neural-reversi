...
use reversi_core::eval::constants;
...
fn load_main_network() -> Network {
    ...
    let eval_path = format!("{}/../{}", manifest_dir, constants::EVAL_FILE_NAME);
    let bytes =
...
fn load_small_network() -> NetworkSmall {
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let eval_path = format!("{}/../{}", manifest_dir, constants::EVAL_SM_FILE_NAME);
    let bytes =

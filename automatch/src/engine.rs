...
/// 
/// ```no_run
/// # use automatch::engine::GtpEngine;
...
/// ).unwrap();
/// 
/// let name = engine.name().unwrap();
/// println!("Engine name: {}", name);
/// 
/// engine.clear_board().unwrap();
/// let move_str = engine.genmove("black").unwrap();
/// println!("Engine played: {}", move_str);
/// ```
...
impl GtpEngine {
    ...
    pub fn send_command(&mut self, command: &str) -> Result<String> {
        let stdin = self
            ...
            .stdin
            .as_mut()
            .ok_or_else(|| AutomatchError::Engine("Failed to open stdin".to_string()))?;

        writeln!(stdin, "{}", command)?;
        stdin.flush()?;

        let stdout = self.process.stdout.as_mut()
            .ok_or_else(|| AutomatchError::Engine("Failed to open stdout".to_string()))?;
        ...
        loop {
            ...
            if bytes_read == 0 {
                return Err(AutomatchError::Engine("Process closed stdout".to_string()));
            }
    ...
    pub fn name(&mut self) -> Result<String> {
        ...
        let name = if name_response.starts_with("= ") {
            ...
            return Err(AutomatchError::Engine(
